import { J2TWindow } from "./j-2-t-window.js";
import {
  folderExists,
  isFoundry8,
  parseTable,
    ...
    if (isItem(Object.keys(table.rows[0]))) {
      for (const item of table.rows) {
        if (options.entityType === "Text") {
          ...
          id = item.item.id;
          collection =
            options.entityType === "Text" ? null : options.entityType;
          resultType = 1;
        }
      ...
      for (const row of table.rows) {
        if (options.entityType === "Text") {
          for (let [key, value] of Object.entries(row)) {
            ...
            } else weight = evaluateDiceTerm(value);
          }
...
const mapEntitiesToTables = async (tablesArray, options) => {
  ...
  for (let a = 0; a < tablesArray.length; a++) {
    ...
    for (let i = 0; i < tablesArray[a].rows.length; i++) {
      ...
      if (
        ...
        options.ifItemExists === "duplicateItem"
      ) {
  ...
  if (folder.content.length === 0) folder.delete();
};
...
const createTableObjects = async (data, tablesArray, options) => {
  ...
  for (const table of tables) {
    ...
    if (tableOject.rows.length === 0) return;
